...
Interrupt:
    ...
    ;BSF PORTB, 0 ; Debug: Toggle ((PORTC) and 07Fh), 0 on ISR entry (confirms interrupts fire)
    NOP
    ;BCF PORTC, 0
    BTFSS PIR1, 3 ; ((PIR1) and 07Fh), 3? (Bank 0)
    GOTO CHECK_BCL
    BCF PIR1, 3 ; Clear ((PIR1) and 07Fh), 3
    BSF STATUS, 5 ; Bank 1 for SSPSTAT/((SSPCON) and 07Fh), 6/((SSPCON) and 07Fh), 7
    BCF STATUS, 6
    BTFSC SSPSTAT, 4 ; Fix: Check ((SSPCON) and 07Fh), 6=1? (receive overflow ? NACK cause)
    CLRF SSPSTAT ; Clear ((SSPCON) and 07Fh), 6/((SSPCON) and 07Fh), 7 (forces ACK on next)
    BTFSC SSPSTAT, 7 ; Check ((SSPCON) and 07Fh), 7=1? (write collision)
    CLRF SSPSTAT ; Clear it
    BTFSC SSPSTAT, 2 ; D/A=1? (data phase)
    GOTO READ_DATA
    BCF STATUS, 5 ; Back to Bank 0
    BCF STATUS, 6
    MOVF SSPBUF, W ; Read address to clear ((SSPSTAT) and 07Fh), 0 (enables data ACK)
    GOTO INTERRUPT_END

READ_DATA:
    BCF STATUS, 5 ; Bank 0
    BCF STATUS, 6
    MOVF SSPBUF, 0 ; Read data ? clears ((SSPSTAT) and 07Fh), 0 ? auto-ACK
    MOVWF RECEIVED_DATA
    GOTO INTERRUPT_END

CHECK_BCL:
    BTFSS PIR2, 3 ; ((PIR2) and 07Fh), 3?
    GOTO INTERRUPT_END
    BCF PIR2, 3
